...
FROM customer_nodes
  ...
  SELECT region_id,
       (SELECT AVG(diff_days) FROM 
           (SELECT DATEDIFF(end_date, start_date) AS diff_days,
                   ROW_NUMBER() OVER (PARTITION BY region_id ORDER BY DATEDIFF(end_date, start_date)) AS row_num,
                   COUNT(*) OVER (PARTITION BY region_id) AS total_count
            FROM customer_nodes) AS subquery
        WHERE row_num IN (FLOOR(0.5 * total_count), CEIL(0.5 * total_count))) AS median_percentile_50,
       ...
       (SELECT AVG(diff_days) FROM 
           (SELECT DATEDIFF(end_date, start_date) AS diff_days,
                   ...
                   COUNT(*) OVER (PARTITION BY region_id) AS total_count
            FROM customer_nodes) AS subquery
        WHERE row_num IN (FLOOR(0.8 * total_count), CEIL(0.8 * total_count))) AS percentile_80,
       ...
       (SELECT AVG(diff_days) FROM 
           (SELECT DATEDIFF(end_date, start_date) AS diff_days,
                   ...
                   COUNT(*) OVER (PARTITION BY region_id) AS total_count
            FROM customer_nodes) AS subquery
        WHERE row_num IN (FLOOR(0.95 * total_count), CEIL(0.95 * total_count))) AS percentile_95
...
select txn_type,sum(txn_amount) as total_txn_amount,
...
with cte as
(select customer_id,
round(Avg(txn_amount),0) as average_deposit,
count(*) as total_deposite
from customer_transactions
where txn_type = 'deposit'
group by customer_id)

select avg(average_deposit) as final_avg,
avg(total_deposite) as finalcount from cte;
...
##  Query 3  For each month - how many Data Bank customers make more than 1 
##          deposit and either 1 purchase or 1 withdrawal in a single month?


WITH CTE AS (
  SELECT 
    DATE_FORMAT(txn_date, '%Y-%m-01') AS month,  -- Truncating date to the first day of the month
    customer_id,
...
SELECT customer_id, 
       month_name, 
       SUM(deposit_amt - purchase_amt - withdrawal_amt) 
           OVER (PARTITION BY customer_id ORDER BY month_num) AS closing_balance
FROM cte
ORDER BY customer_id, month_num;
...
prev_balance_cte AS (
    SELECT *, 
           LAG(closing_balance, 1, 0) 
               OVER (PARTITION BY customer_id ORDER BY month_num) AS prev_balance
    FROM closing_balance_cte
)

SELECT ROUND(100.0 * COUNT(DISTINCT customer_id) / 
           (SELECT COUNT(DISTINCT ct.customer_id) 
            FROM data_bank.customer_transactions ct), 1) AS percentage
FROM prev_balance_cte
WHERE prev_balance <> 0 
  AND closing_balance > 1.05 * prev_balance;
