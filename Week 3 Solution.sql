...
select distinct count(customer_id) Total_Customers from subscriptions;
...
select 
     date_format(start_date, '%Y-%m-01') as startmonth,
count(customer_id) as trialcount
      from subscriptions
where plan_id = 0
GROUP BY 
...
where date_format(start_date, '%Y-%m-01') > '2020-12-31'
       group by plan_name;
...
(select count(distinct customer_id ) 
        from subscriptions) as totalcount,
round((count(distinct customer_id) / (select count(distinct customer_id) 
        from subscriptions)) *100 ,1) as churnrate
from subscriptions
        where plan_id = 4;

select
      (count(distinct customer_id) / (select count(distinct customer_id) 
...
round(((count(customer_id)) / (select count(distinct customer_id) from cte)) * 100,1) as customerperc
         from cte
where rownumber = 2
         group by plan_name;
...
select 
plan_name,
       count(customer_id) as allcustomer,
round((count(customer_id)/(select count(distinct customer_id) from cte))*100,1) as totalcust
		from cte
inner join plans as p on cte.plan_id = p.plan_id
        where rown = 1 
group by plan_name;

## Query 8    How many customers have upgraded to an annual plan in 2020?  done

select count(customer_id) as
        annual_customer_upgraded
from subscriptions as s
        inner join plans as p on 
p.plan_id = s.plan_id
...
SELECT 
    AVG(DATEDIFF(a.annual_start, t.trial_start)) AS avg_days_to_annual
FROM trial AS t
INNER JOIN annual AS a ON t.customer_id = a.customer_id;
...
SELECT 
    CASE 
        WHEN DATEDIFF(a.annual_start, t.trial_start) BETWEEN 0 AND 30 THEN '0-30 days'
        WHEN DATEDIFF(a.annual_start, t.trial_start) BETWEEN 31 AND 60 THEN '31-60 days'
        WHEN DATEDIFF(a.annual_start, t.trial_start) BETWEEN 61 AND 90 THEN '61-90 days'
        WHEN DATEDIFF(a.annual_start, t.trial_start) BETWEEN 91 AND 120 THEN '91-120 days'
        WHEN DATEDIFF(a.annual_start, t.trial_start) > 120 THEN '120+ days'
        ELSE 'Unknown'
    END AS period_range,
    COUNT(*) AS customer_count,
    AVG(DATEDIFF(a.annual_start, t.trial_start)) AS avg_days_in_period
FROM trial AS t
INNER JOIN annual AS a ON t.customer_id = a.customer_id
...
with pmonth as (
        select customer_id,
start_date as p_monthly_start from subscriptions
        where plan_id = 2
)

,basicmon as (
select start_date as bas_monthly_start from subscriptions 
where plan_id = 1
)
select p.customer_id ,p_monthly_start,bas_monthly_start
from pmonth as p
inner join basicmon as B on p.customer_id = B.customer_id
where pmonth < basicmon 
and year(bas_monthly_start) = 2020
